...
contract Staking is IStaking, Ownable, ReentrancyGuard, ERC20, Pausable {
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        totalSupplyBP -= variables.balanceBP;
    ...
    function vest(uint256 amount) public nonReentrant updateReward(msg.sender) {
        amount *= AMOUNT_MULTIPLIER;
    ...
    function compoundBP() external updateReward(msg.sender) {
        ...
        if (bonusPoints > 0) {
            ...
            totalSupplyLP += bonusPoints;

            variables.balanceBP = 0;
            totalSupplyBP -= bonusPoints;
    ...
    function updateBonusPoints(address account) internal {
        ...
        uint256 increaseOfBP =
            ((block.timestamp - variables.userLastUpdateTime) * variables.balanceLP) / ONE_YEAR_IN_SECS;
    ...
    function updateVesting(address account) internal {
        ...
        uint256 increaseOfNC = (
            (Math.min(block.timestamp, variables.vestingFinishTime) - variables.userLastUpdateTime)
                * Math.min(variables.balanceVST, variables.balanceLP / VESTING_CONST)
        ) / ONE_YEAR_IN_SECS;
    ...
    function notifyNativeRewardAmount(uint256 amount) external payable onlyOwner updateReward(address(0)) {
        if (msg.value != amount) revert InvalidRewardAmount();
    ...
    function setTokenRewardsDuration(uint256 _duration) external onlyOwner {
        if (block.timestamp <= tokenPeriodFinish) revert IncompletePrevRewardsPeriod();
    ...
    function setNativeRewardsDuration(uint256 _duration) external onlyOwner {
        if (block.timestamp <= nativePeriodFinish) revert IncompletePrevRewardsPeriod();
    ...
    /// @notice Returns the scaled bonus points balance of an account
    /// @param account Address to check
    /// @return Scaled bonus points balance
    function getScaledBalanceBPOf(address account) external view returns (uint256) {
        return userVariables[account].balanceBP;
    }
    ...
    /// @notice Returns the scaled LP balance of an account
    /// @param account Address to check
    /// @return Scaled LP balance
    function getScaledBalanceLPOf(address account) external view returns (uint256) {
        return userVariables[account].balanceLP;
    }
    ...
    /// @notice Returns the scaled ST balance of an account
    /// @param account Address to check
    /// @return Scaled ST balance
    function getScaledBalanceSTOf(address account) external view returns (uint256) {
        return userVariables[account].balanceST;
    }
    ...
    function getTotalSupplyLP() external view returns (uint256) {
        return totalSupplyLP / AMOUNT_MULTIPLIER;
    }

    /// @notice Returns the scaled total supply of LP tokens
    /// @return Scaled total LP supply
    ...
    function getTotalSupplyBP() external view returns (uint256) {
        return totalSupplyBP / AMOUNT_MULTIPLIER;
    }

    /// @notice Returns the scaled total supply of BP tokens
    /// @return Scaled total BP supply
    ...
    function getTotalSupplyST() external view returns (uint256) {
        return totalSupplyST / AMOUNT_MULTIPLIER;
    }

    /// @notice Returns the scaled total supply of ST tokens
    /// @return Scaled total ST supply
    function getScaledTotalSupplyST() external view returns (uint256) {
        return totalSupplyST;
    }

    /// @notice Returns the VST balance of an account
    /// @param account Address to check
    /// @return VST balance
    function balanceVST(address account) external view returns (uint256) {
        return userVariables[account].balanceVST / AMOUNT_MULTIPLIER;
    }

    /// @notice Returns the scaled VST balance of an account
    /// @param account Address to check
    /// @return Scaled VST balance 
    function getScaledBalanceVST(address account) external view returns (uint256) {
        return userVariables[account].balanceVST;
    }

    /// @notice Returns the VST stored balance of an account
    /// @param account Address to check
    /// @return VST stored balance
    function balanceVSTStored(address account) external view returns (uint256) {
        return userVariables[account].balanceVSTStored / AMOUNT_MULTIPLIER;
    }

    /// @notice Returns the scaled VST stored balance of an account
    /// @param account Address to check
    /// @return Scaled VST stored balance
    function getScaledBalanceVSTStored(address account) external view returns (uint256) {
        return userVariables[account].balanceVSTStored;
    }
    ...
    function earned(address account) external view returns (uint256) {
        return userVariables[account].rewards;
    }
